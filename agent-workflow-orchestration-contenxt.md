...
- `agents/` – Role definitions (system prompts, allowed delegations, model choices) for specialized workers such as `programmer`, `documentor`, `Explore`, `Plan`, `junior-engineer`, `senior-engineer`, and `orchestrator`. Main agents delegate by referencing `@agent-name` to spin up task-specific collaborators.
  - The root agent's system prompt incorporates `output-styles/main.md`.
- `hooks/pre-tool-use/agent-interceptor.js` – Intercepts task spawns, enforces recursion limits and allowlists, and wires asynchronous delegation. Spawned agents via klaude execute in background and return output directly through the command.
...
- Async subagents handle parallelizable or "busy work" tasks, with klaude returning their output directly when complete.
...
- Delegations should bundle precise context (file paths, relevant docs, command references) so subagents can work independently with minimal follow-up.
  - Workflows specific to subagents should be handled using the agent's .md file in `agents/`.
- klaude command returns agent output directly—no file polling required.
